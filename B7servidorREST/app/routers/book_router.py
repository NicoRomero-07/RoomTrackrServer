...
@router.get(
    "/{id}", response_description="Get a single booking", response_model=Booking
)
...
def create_household(request: Request, Booking: Booking = Body(...)):
    ...
    Booking['host']['host_username'] = household.get('host').get('host_username')
    Booking['host']['host_email'] = household.get('host').get('host_email')
...
def delete_all_household(request: Request, response: Response):
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail=f"Booking not found"
    )
...
@router.get(
    "_actives",
    response_description="List all active bookings",
    response_model=List[Booking],
)
def list_active_bookings(request: Request):
    active_bookings = list(
        request.app.database["booking"]
        .find({"ending": {"$gte": str(datetime.now())}}, limit=100)
        .sort("start", -1)
    )
...
def list_incative_bookings(request: Request):
    active_bookings = list(
        request.app.database["booking"]
        .find({"ending": {"$lte": str(datetime.now())}}, limit=100)
        .sort("start", -1)
    )
    return active_bookings
...
@router.get(
    "/from_user/{username}",
    response_description="Get the bookings of an user ordered by date",
)
def get_ordered_bookings_by_username(
    username: str, request: Request, response: Response
):

    bookings = list(
        request.app.database["booking"]
        .find({"renter.renter_username": username}, {"_id": 0}, limit=100)
        .sort("start", -1)
    )
    return bookings
...
@router.get(
    ...
    response_description="Get the bookings of a household ordered by date",
)
def get_ordered_bookings_by_household_id(
    household_id: str, request: Request, response: Response
):

    bookings = list(
        request.app.database["booking"]
        .find({"household.id": household_id}, {"_id": 0}, limit=100)
        .sort("start", -1)
    )
    return bookings
